...
_sql={
	'output_ALL' : """
		...
		join (
			...
			from schd
			group by voyageid
			having max(lastupdated)::date = %(startdate)s::date
...
def outputfile_ALL(dbh,outputdir, archivedir=None, startdate=None):
	...
	if os.path.isdir(outputdir):
		...
		if startdate is not None:			
			timestr=startdate.strftime("%Y-%m-%d_%H-%M-%S")
			
		csvfilename=os.path.join(outputdir,'ssm_backdate_'+timestr+'.csv')
		trigfilename=os.path.join(outputdir,'ssm_backdate_'+timestr+'.trg')

		archivefilename=None
		if archivemode:
			archivefilename=os.path.join(archivedir,'ssm_backdate_'+timestr+'.csv')
		
		module_logger.info("Output to %s %s",csvfilename, trigfilename)
		
		csr=dbh.cursor()

		csr.execute( _sql['output_ALL'], { 'startdate' : startdate } )
...
def main():
	...
	# must have at least one date (in YYYY-MM-DD) on command line
	...
	if args.dates is not None:
		for d in args.dates:
			status, message=outputfile_ALL(dbh, args.folder, args.archive, d)
	else:
